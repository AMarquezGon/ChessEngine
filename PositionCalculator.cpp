...
std::list<Board> PositionCalculator::nextPossiblePositions(Board board)
{
	...
	if (castleKingsideCheck(board))
	{
		Board newPosition{ board, 0 };
		BitBoard myKing = newPosition.m_pieces[Board::king + Board::max_pieces * board.m_currentTurn];
		BitBoard myRook = myKing << 3;
		newPosition.m_castling &= 3 << ((1-board.m_currentTurn) * 2); // basically take 11 in binary and shift it depending on what color just played. If white castled then you are taking castling AND 1100. If black castled then you get castling AND 11.
		newPosition.m_pieces[Board::king + Board::max_pieces * board.m_currentTurn] <<= 2;
		newPosition.m_pieces[Board::rook + Board::max_pieces * board.m_currentTurn] = (newPosition.m_pieces[Board::rook + Board::max_pieces * board.m_currentTurn]&(~myRook))|(myKing<<1); //eliminate rook from corner and place it where the king used to be shifted by 1
		addPosition(newPosition, possiblePositions);
	}

	if (castleQueensideCheck(board))
	{
		Board newPosition{ board, 0 };
		BitBoard myKing = newPosition.m_pieces[Board::king + Board::max_pieces * board.m_currentTurn];
		BitBoard myRook = myKing >> 4;
		newPosition.m_castling &= 3 << ((1 - board.m_currentTurn) * 2); // basically take 11 in binary and shift it depending on what color just played. If white castled then you are taking castling AND 1100. If black castled then you get castling AND 11.
		newPosition.m_pieces[Board::king + Board::max_pieces * board.m_currentTurn] >>= 2;
		newPosition.m_pieces[Board::rook + Board::max_pieces * board.m_currentTurn] = (newPosition.m_pieces[Board::rook + Board::max_pieces * board.m_currentTurn] & (~myRook)) | (myKing >> 1); //eliminate rook from corner and place it where the king used to be shifted by 1
		addPosition(newPosition, possiblePositions);
